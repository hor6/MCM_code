...
###############################################################################
####### Adicionado por Rodrigo Ho para consertar o Loop########################
## Loop over subjects in the trial

N_p<- length(brim3[,"SEX"])
xty <- seq(0, 30, by = 0.2)
rate_vec <- matrix(0,nrow = N_p, ncol = 1)
for (i in 1:N_p) {
  country_in_use <- as.character(brim3[i, "COUNTRY"])
  year_in_use <- brim3[i, "YEAR"]
  gender_in_use <- brim3[i, "SEX"]
  ...
  if (gender_in_use == "MALE" | gender_in_use == "M") {
    ...
    rate_vec[i] <- rate_c_years(as.numeric(brim3[i, "AGE"]) +
                                  as.numeric(brim3[i, "TIME"]), table_in_use)
    scvy_all <- Surv_cv_years(as.numeric(brim3[i, "AGE"]) +
                                xty, table_in_use)
  ...
  if (gender_in_use == "FEMALE" | gender_in_use == "F") {
    ...
    rate_vec[i] <- rate_c_years(as.numeric(brim3[i, "AGE"]) +
                                  as.numeric(brim3[i, "TIME"]), table_in_use)
    scvy_all <- Surv_cv_years(as.numeric(brim3[i, "AGE"]) + xty, table_in_use)
    
  }
  ret_obj <- list()
  ret_obj$rate_vec <- rate_vec
  brim3$rate_mod <-rate_vec
}
########################################################################
#####Added
